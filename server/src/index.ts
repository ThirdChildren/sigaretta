...
io.on("connection", (socket) => {
  ...
  socket.on("room:start", (data: { code: string }, ack) => {
    ...
    if (!player?.isHost)
      return ack?.({
        ok: false,
        error: "Solo il creatore della stanza può avviare la partita.",
      });
  ...
  socket.on("answer:submit", (data: { code: string; text: string }, ack) => {
    ...
    if (room.status === "reveal") {
      const stories = revealStories(room);
      const assignment = assignStoriesToPlayers(room);
      for (const player of room.players) {
        const storyId = assignment.get(player.id);
        const story = stories.find((s) => s.id === storyId);
        io.to(player.id).emit("room:reveal", story);
      }
    } else {
      sendPrompts(room);
    }
