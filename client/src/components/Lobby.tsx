  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-900 via-slate-800 to-amber-900 p-4">
      <div className="w-full max-w-md rounded-3xl bg-white p-8 shadow-2xl">
        <h1 className="mb-1 text-center text-2xl font-extrabold text-slate-800">Sala d'attesa</h1>
        <p className="mb-5 text-center text-sm text-slate-500">Condividi questo codice con gli altri giocatori</p>
        ...
        <div className="mb-4 flex items-center justify-between text-xs font-semibold uppercase tracking-wide text-slate-400">
          ...
          {!canStart && <span className="text-amber-600">Minimo 3 per iniziare</span>}
        </div>
          ...
          {room.players.map((p) => (
            <li
              ...
              className={`flex items-center justify-between rounded-xl border px-4 py-3 ${
                p.id === myId ? 'border-amber-300 bg-amber-50' : 'border-slate-100 bg-slate-50'
              }`}
            >
              <span className="flex items-center gap-2 font-medium text-slate-700">
                <PersonIcon fontSize="small" className="text-slate-400" />
                {p.name}
                {p.id === myId && <span className="text-xs text-amber-500">(tu)</span>}
              </span>
              <span className="flex items-center gap-2">
                {!p.connected && <WifiOffIcon fontSize="small" className="text-red-400" />}
                {p.isHost && <StarIcon fontSize="small" className="text-amber-500" />}
              </span>
            </li>
          ))}
        </ul>
        ...
        {isHost ? (
          ...
          <p className="text-center text-sm text-slate-400">In attesa che il padrone di casa inizi la partita...</p>
        )}
      </div>
    </div>
  )
